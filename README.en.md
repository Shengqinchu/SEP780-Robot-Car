...
| Manual control | IR buttons or an Android analog joystick; differential mixing, ramped acceleration, release-to-stop, and a selectable 110–180 PWM app limit |
...
**Progress:** Servo, ranging, wheel direction, BLE, and the Normal line profile have completed physical integration; on the same course, the user observed a higher success rate in Normal than Pulse. Line behavior remains an 80/80 ms PWM 120 centered pulse with PWM 150 turns and is unchanged by this remote-control adjustment. The controller uses one atomic `R` command, a 20 Hz latest-value queue, an analog differential joystick, a default limit of 150, acceleration limiting, and neutral-before-reverse. It also adds a hold-to-sound horn with a 1.5-second disconnect lease. The app now offers a selectable limit of 110–180. The firmware wire format still accepts up to 200, but the motor I/O clips physical writes to 180. On 2026-09-21 the joystick build was flashed to COM5 and installed over the previous Redmi app; both language layouts and the USB-only `idle / 0 / 0` safe default were accepted. The user reported that powered operation was usable overall and that five runs on one white-backed black-line course were normal. The supplied video shows movement on that course and a connected phone, but it uses the older circular-control app and does not validate the new 180 ceiling or joystick build on hardware. Occasional ignored input with `command rejected` remains unlocalized. Low-voltage, sensor-fault, emergency-stop, and communication-lease interlocks remain active.
