...
def parse_telemetry(line: bytes):
    ...
    if ms > 0xFFFFFFFF or state >= len(STATES) or mm > 4000 or mv > 20000 or abs(left) > 180 or abs(right) > 180:
        return None
...
def encode(sequence: int, action: str, left: int = 0, right: int = 0) -> bytes:
    if type(sequence) is not int or not 1 <= sequence <= 65535:
        raise ValueError("Sequence must be an integer in 1..65535.")
    if any(type(value) is not int or not -150 <= value <= 150 for value in (left, right)):
        raise ValueError("Wheel PWM must be integer values in -150..150.")
    commands = {"stop": "STOP", "line": "ARM LINE", "manual": "ARM MANUAL", "ping": "PING", "status": "STATUS",
                "drive": f"DRIVE {left} {right}"}
