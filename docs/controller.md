...
Freenove 官方定义黑色或无反射物为高电平、白色为低电平；硬件层使用 `black_reads_high = true`，将原始三路值直接作为“黑线位 = 1”交给控制器。实车逐档观察确认 PWM 50–100 不能可靠起步，PWM 120 作为循迹脉冲的下一档启动幅值。固件兼容的手动协议数值范围为 50–200，电机 I/O 实际截断到 180；App 为避免已知的冷启动死区，把用户可选上限限制为 110–180，默认 150。摇杆经过死区、曲线和左右差速混合后输出，低于起步门槛的非零量会抬到 110；这仍是开环 PWM，不是实际速度闭环。
