...
| Item | Status |
| --- | --- |
| Analog joystick, eight remote limits, atomic protocol, latest-value BLE queue, acceleration ramp, and horn lease | Implemented and offline-tested |
| Java unit tests, lint, and debug APK | 17/17 unit tests and lint pass; the 180-limit APK SHA-256 is `8655a8448522f10c86966e18d8a863bf31342b5e85163f24457046a1beb0935c` |
| Installed on an Android phone | The previous 200-limit joystick build was installed on Redmi; this 180-limit build has not been installed or tested over BLE |
...
| Phone remote and line profile | The previous build completed a powered integration session and the user reports overall usable behavior; an occasional ignored input with `command rejected` remains without a failure-time ACK/telemetry trace |
