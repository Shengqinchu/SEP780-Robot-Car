...
| 手动控制 | 红外按键或安卓模拟摇杆控制；差速混控、渐进加速、松手停车，App 可选上限 110–180 PWM |
...
**当前进度：**舵机、测距、四轮方向、BLE 和“正常”循迹已完成实车联调；用户在同一场地观察到正常档成功率明显高于步进档。循迹继续使用中线 PWM 120 的 80/80 ms 脉冲和 PWM 150 转弯，不受本轮遥控调整影响。新版遥控采用单条原子 `R` 指令、20 Hz 最新值队列、模拟差速摇杆、默认 150 的上限、渐进加速和反向前归零，并加入带 1.5 秒失联租约的按住鸣笛。App 可选上限现为 110–180；固件协议仍接受到 200，但电机 I/O 实际截断到 180。2026-09-21 的摇杆版已刷入 COM5 并覆盖安装到 Redmi；中英文单页 UI 与 USB-only `idle / 0 / 0` 默认安全状态已验收。用户报告带电综合操作总体可用，并报告同一白底黑线路线完成 5 次测试、均正常；提供的录像显示路线运动和蓝牙连接，但使用的是旧圆盘 App，不能作为本次 180 上限或新版摇杆的实车验收。偶发不响应并显示 `command rejected` 的原因仍未定位。低压、传感器故障、急停和通信租约仍然有效。
